...
const LOCAL_FORTUNE_MODEL = "deepseek-r1:8b";
...
function handleClick(event) {
  ...
  if (promptButton) {
    void submitFollowUp(promptButton.dataset.chatPrompt);
    return;
  }
  ...
  try {
    const response = await fetch("/api/fortune-chat", {
      ...
      body: JSON.stringify({
        ...
        profile: {
          gender: state.currentReading.genderLabel,
          city: state.currentReading.city,
          birthDate: state.currentReading.solarText.slice(0, 10),
          birthTime: state.currentReading.solarText.slice(11, 16),
          starSign: state.currentReading.starSign,
        },
        readingSummary: buildReadingSummary(state.currentReading),
    ...
    state.messages = [
      ...
      {
        role: "assistant",
        text: String(payload.reply || "").trim() || "山风过耳，此刻尚未得辞。",
      },
    ];
    state.chatModel = payload.model || state.chatModel;
  } catch (error) {
    state.chatError =
...
function buildReadingSummary(reading) {
  return [
    `缘主：${reading.displayName}`,
    `造式：${reading.genderLabel}`,
    `出生城市：${reading.city}`,
    `阳历：${reading.solarText}`,
    `农历：${reading.lunarText}`,
    `星座：${reading.starSign}`,
    `日主：${reading.dayStem}${reading.dayElement}`,
    `四柱：${reading.pillars.map((pillar) => `${pillar.label}${pillar.ganzhi}`).join("，")}`,
    `喜用：${reading.favorable.useful.join("、")}`,
    `忌神：${reading.favorable.avoid.join("、")}`,
    `盘面初看：${reading.prose.overall}`,
    `事业：${reading.prose.career}`,
    `姻缘：${reading.prose.relationship}`,
    `流年：${reading.prose.annual}`,
    `星座分析：${reading.prose.astro}`,
    `化解之道：${reading.prose.remedy}`,
    `今日运势：${reading.fortuneProfile.todaySummary}${reading.fortuneProfile.todayAdvice}`,
    `月运走势：${reading.fortuneProfile.monthlySummary}`,
  ].join("\n");
}
